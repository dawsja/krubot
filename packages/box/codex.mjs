...
export class CodexSession extends BridgedSession {
  ...
  async ensure({ model, effort, systemPrompt, tools, permission = "ask", mcpServers = [], access = { kind: "plan" } }) {
    ...
    if (!fs.existsSync(codexHome)) {
      ...
      if (this.uid !== undefined) fs.chownSync(codexHome, this.uid, this.gid);
    }
  ...
  onExit(child, code, signal) {
    ...
    if (turn) {
      this.turn = null;
      turn.finish({ ok: false, text: turn.text, stopReason: turn.interrupted ? "interrupted" : "exited", cost: null, usage: null, error: turn.interrupted ? "The turn was interrupted" : this.stderr.trim().slice(0, 500) || `Codex exited (${signal ?? code})` });
    }
  }
  ...
  turnWith({ prompt, images, timeoutMs, onEvent, systemPrompt, signal }) {
    ...
    return new Promise((resolve) => {
      ...
      this.rpc
        .request("turn/start", { threadId: this.sessionId, input: codexInput(text, images), ...(this.effort ? { effort: this.effort } : {}) })
        .then((started) => {
