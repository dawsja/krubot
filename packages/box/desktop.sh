...
settings() {
  ...
  # GNOME's own wallpaper. Homes are volumes, so one from an older box that
  # set its own is put back to the default.
  for key in picture-uri picture-uri-dark picture-options color-shading-type primary-color; do
