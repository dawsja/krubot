...
test("Grok runs without borrowing Claude's config, on the proxy for an API", () => {
  ...
  assert.deepEqual(grokArgs({ model: "", permission: "ask" }), ["agent", "stdio"]);
  assert.deepEqual(grokArgs({ model: "grok-build", permission: "full" }), ["agent", "--always-approve", "--model", "grok-build", "stdio"]);
});
