...
export function protocolLogger(dir, id) {
  ...
  return (direction, payload) => {
    try {
      ...
      // The folder is in a person's home and this runs as root: a link
      // anywhere on the way, or at the file, and nothing is written.
      if (fs.realpathSync(dir) !== path.resolve(dir)) return;
      try {
        if (fs.lstatSync(file).size > MAX_LOG_BYTES) fs.renameSync(file, `${file}.1`);
      } catch {
        /* no file yet */
      }
      ...
      try {
        fs.writeSync(fd, `${JSON.stringify({ at: new Date().toISOString(), direction, payload })}\n`);
      } finally {
        fs.closeSync(fd);
      }
    } catch {
 ...
 */
/**
 * What a turn posts as its reply. Normally the words after the last tool
 * call: what a model says before reaching for a tool is narration. A model
 * can also write its answer first and end on tool calls (a Chief of Staff
 * handing out a wave of briefs), leaving nothing after the last one; then
 * the answer is every word of the turn, or the person gets a lead-in cut
 * off at "relaying that…".
 */
export class ReplyText {
  /** Text between tool calls, oldest first; the last one is still being written. */
  segments = [""];

  /** Streamed text, joined as it came. */
  chunk(text) {
    this.segments[this.segments.length - 1] += text;
  }

  /** A whole block of text (a message or a content block), set apart from the one before. */
  block(text) {
    if (!text.trim()) return;
    const last = this.segments.length - 1;
    this.segments[last] = this.segments[last].trim() ? `${this.segments[last].trimEnd()}\n\n${text.trim()}` : text;
  }

  /** The model reached for a tool: what it wrote so far is behind it. */
  tool() {
    if (this.segments[this.segments.length - 1].trim()) this.segments.push("");
  }
  ...
  text() {
    const after = this.current().trim();
    if (after) return after;
    return this.segments.map((s) => s.trim()).filter(Boolean).join("\n\n");
  }
}
