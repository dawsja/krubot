...
export function grokArgs({ model, permission }) {
  const args = ["agent"];
...
export class GrokSession extends BridgedSession {
  ...
  async ensure({ model, systemPrompt, tools, permission = "ask", mcpServers = [], access = { kind: "plan" } }) {
    ...
    const argsKey = createHash("sha256")
      .update(JSON.stringify({ engine: "grok", model, permission, mcpServers, access, tools: tools.map((t) => t.name).sort() }))
      .digest("hex")
      .slice(0, 16);
    ...
    const args = grokArgs({ model, permission });
    this.stderr = "";
    this.log("spawn", { args });
  ...
  onExit(child, code, signal) {
    ...
    if (turn) {
      this.turn = null;
      turn.finish({ ok: false, text: turn.reply.text(), stopReason: turn.interrupted ? "interrupted" : "exited", cost: null, usage: null, error: turn.interrupted ? "The turn was interrupted" : this.stderr.trim().slice(0, 500) || `Grok exited (${signal ?? code})` });
    }
  }
    ...
    if (method === "session/update") {
      switch (update.sessionUpdate) {
        case "agent_message_chunk":
          if (update.content?.type === "text" && typeof update.content.text === "string") turn.reply.chunk(update.content.text);
          return;
        case "tool_call": {
          // What it said before reaching for a tool shows as activity; the reply decides what it posts.
          if (turn.reply.current().trim()) turn.onEvent({ type: "activity", line: oneLine(turn.reply.current(), 200) });
          turn.reply.tool();
  ...
  turnWith({ prompt, images, timeoutMs, onEvent, systemPrompt, signal }) {
    ...
    return new Promise((resolve) => {
      const turn = {
        ...
        reply: new ReplyText(),
        usage: null,
        // The person's messages for this turn, sent as the next prompt once the running one is cancelled.
        steers: [],
        finish: (result) => {
          clearTimeout(timer);
      ...
      const send = (content) =>
          this.rpc
          .request("session/prompt", { sessionId: this.sessionId, prompt: content })
          .then((done) => {
            if (this.turn !== turn) return;
            // Cancelled for a steer: the same turn goes on with the person's words.
            if (turn.steers.length && !turn.interrupted && this.rpc) {
              const steers = turn.steers.splice(0);
              turn.reply.tool();
              this.log("in", { steer: steers.length });
              send(grokPrompt(steers.join("\n\n"), [], this.imagesSupported));
              return;
            }
            this.turn = null;
            ...
            turn.finish({ ok: ok && !turn.interrupted, text: turn.reply.text(), stopReason, cost: null, usage: turn.usage, error: ok ? null : stopReason === "interrupted" || stopReason === "cancelled" ? "The turn was interrupted" : `Grok stopped (${stopReason})` });
          })
          .catch((error) => {
            if (this.turn !== turn) return;
            this.turn = null;
            turn.finish({ ok: false, text: turn.reply.text(), stopReason: "error", cost: null, usage: null, error: error.message });
          });
      send(grokPrompt(text, images, this.imagesSupported));
    });
  }

  /**
   * ACP has no way into a running prompt, so a steer cancels it and sends
   * the person's words as the next prompt of the same turn; the session
   * keeps everything done so far. Not while Grok waits on an approval or a
   * tool, which the cancel would leave hanging: the message waits instead.
   */
  async steer(text) {
    const turn = this.turn;
    if (!turn || turn.interrupted || !this.rpc || this.pendingCalls.size > 0) return false;
    turn.steers.push(text);
    if (turn.steers.length === 1) this.rpc.notify("session/cancel", { sessionId: this.sessionId });
    return true;
  }
