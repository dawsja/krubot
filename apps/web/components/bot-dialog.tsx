  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (existing) {
        ...
        // A bot takes on the Chief of Staff's job when you tell it so in conversation.
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>A name, a job, and a way of working. A focused bot beats a catch-all one.</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit}>
          <FieldGroup className="gap-5">
            ...
            <Field>
              ...
              <Textarea id="bot-description" value={form.description} onChange={(e) => set("description", e.target.value)} rows={5} maxLength={BOT_LIMITS.description} placeholder="What it owns, what it must ask before doing, and what it returns. This becomes its SOUL.md." />
            </Field>
            <Field>
              ...
              <FieldDescription>{APPROVAL_LEVEL_LABELS[form.approval].detail} Every bot runs on the model chosen under Settings. Any bot can add teammates and hand work to them; tell one it is your Chief of Staff in its conversation to make it the one that coordinates.</FieldDescription>
            </Field>

            <FieldSet>
              <FieldLegend variant="label">Connected apps this bot may use</FieldLegend>
              {connections.length === 0 && servers.length === 0 ? (
                <FieldDescription>No apps connected yet. Connect Gmail, Slack, GitHub and more under Settings → Apps.</FieldDescription>
              ) : (
            ...
            <FieldLabel htmlFor="bot-notify">
              <Field orientation="horizontal">
                <FieldContent>
                  ...
                  <FieldDescription>A notification when it finishes or needs you. Turn them on under Settings first.</FieldDescription>
                </FieldContent>
