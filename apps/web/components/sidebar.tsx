...
import { MessageSquareText, Pin, Plus, Search, SquareKanban, Users } from "lucide-react";
...
import { Sidebar, SidebarContent, SidebarFooter, SidebarGroup, SidebarGroupContent, SidebarHeader, SidebarMenu, SidebarMenuBadge, SidebarMenuButton, SidebarMenuItem, SidebarMenuSkeleton, useSidebar } from "@/components/ui/sidebar";
...
/** How many things on the board wait on you, with the amber dot approvals carry; nothing when none do. */
export function NeedsYouCount({ className }: { className?: string }) {
  const { needsYouCount } = useStore();
  if (!needsYouCount) return null;
  return (
    <SidebarMenuBadge className={cn("gap-1.5 text-muted-foreground", className)} aria-label={`${needsYouCount} waiting on you`}>
      <span className="size-2 rounded-full bg-amber" aria-hidden="true" />
      {needsYouCount}
    </SidebarMenuBadge>
  );
    ...
    <Sidebar collapsible="offcanvas" className="bg-background [&_[data-sidebar=sidebar]]:bg-background">
      ...
      <SidebarFooter className="border-t">
        <SidebarMenu className="gap-0.5">
          <SidebarMenuItem>
            <SidebarMenuButton isActive={pathname.startsWith("/app/board")} render={<Link href="/app/board" />} className="h-10 gap-3 rounded-xl px-2.5 text-[13.5px] font-medium data-active:bg-card data-active:shadow-subtle hover:bg-card/60">
              <SquareKanban aria-hidden="true" />
              <span>Board</span>
            </SidebarMenuButton>
            <NeedsYouCount className="right-2 peer-data-[size=default]/menu-button:top-2.5" />
          </SidebarMenuItem>
