 ...
 * a colour.
 */
export function ActionCard({ icon, title, children, fill = false, className }: { icon: ReactNode; title: ReactNode; children: ReactNode; /** Take the whole width, as on the board, rather than sit in a conversation like a message. */ fill?: boolean; className?: string }) {
  return (
    <Bubble variant="outline" className={cn(fill ? "w-full max-w-none" : "max-w-[80%]", className)}>
      <BubbleContent className={cn("flex flex-col gap-2 rounded-2xl border-primary-edge p-3", fill && "w-full")}>
