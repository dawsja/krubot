...
import { defaultOAuthRedirect, joinCommandLine, MCP_OAUTH_REDIRECTS, MCP_TRANSPORTS, type McpOAuthRedirect, type McpServer, type McpTransport, splitCommandLine } from "@krubot/shared";
...
export function McpDialog({ server, onClose, onSaved }: { server: McpServer | null; onClose: () => void; onSaved: () => void | Promise<void> }) {
  ...
  const [command, setCommand] = useState(server?.command ? joinCommandLine([server.command, ...server.args]) : "");
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Give it to a bot in its profile. Writes still ask for your approval.</DialogDescription>
        </DialogHeader>
            ...
            {transport === "http" ? (
              <>
                <Field>
                  ...
                  <FieldDescription>The bots reach it through Kru Bot, which adds the headers below. A server that wants a sign-in gets a button on its row.</FieldDescription>
                </Field>
                <Field>
                ...
                <Field>
                  ...
                  <FieldDescription>Pick localhost for a server that only lets local tools sign in, like Robinhood&apos;s. Otherwise leave it.</FieldDescription>
                </Field>
              </>
            ) : (
              <>
                <Field>
                  ...
                  <FieldDescription>Runs on the computer as the bots&apos; user, in their home. Quote a path with spaces in it.</FieldDescription>
                </Field>
                <Field>
