...
import { Keyboard, Maximize2, Minimize2, MonitorOff, Power, X } from "lucide-react";
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            onClick={onClick}
            aria-label={label}
            className={cn("text-white/60 hover:bg-white/10 hover:text-white", className)}
          />
        }
      >
        {children}
...
/**
 * A phone has no keys to press: its keyboard only opens for something the
 * browser can edit, and the desktop is a canvas. So a field with
 * nothing in it sits behind the picture, the Keyboard button focuses it,
 * and what the phone types into it is sent on as key presses. The field
 * is kept filled with spaces, so a backspace with nothing typed yet still
 * reaches the desktop.
 */
const KEYBOARD_FILLER = "\u00a0\u00a0\u00a0\u00a0";
const KEYSYM_BACKSPACE = 0xff08;
const KEYSYM_RETURN = 0xff0d;
const KEYSYM_TAB = 0xff09;

/** The X keysym for one character the phone typed. */
export function keysymOf(character: string): number {
  if (character === "\n" || character === "\r") return KEYSYM_RETURN;
  if (character === "\t") return KEYSYM_TAB;
  const code = character.codePointAt(0) ?? 0;
  // Latin-1 is its own keysym; everything else is Unicode with the high bit set.
...
export function BoxDesktop({ onClose }: { onClose: () => void }) {
  ...
  /**
   * What the phone typed into the hidden field, on to the desktop as key
   * presses: whatever is past the filler is new text, and a shorter field
   * than the filler is that many backspaces. The field is put back to the
   * filler each time, so it never fills up and never shows a caret moving.
   */
  const onTyped = useCallback(() => {
    ...
    if (typed.length > KEYBOARD_FILLER.length) {
      for (const character of typed.slice(KEYBOARD_FILLER.length)) client.sendKey(keysymOf(character), null);
    } else {
      for (let i = typed.length; i < KEYBOARD_FILLER.length; i += 1) client.sendKey(KEYSYM_BACKSPACE, null);
    }
    field.value = KEYBOARD_FILLER;
    field.setSelectionRange(KEYBOARD_FILLER.length, KEYBOARD_FILLER.length);
  }, []);

  /** The Keyboard button: focusing the field is what opens a phone's keyboard. */
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-2.5">
          ...
          <div className="flex items-center gap-1">
            ...
            {status === "live" ? (
              <IconButton container={frame} label="Keyboard" onClick={openKeyboard} className="hidden pointer-coarse:flex">
        ...
        <div className="relative min-h-0 flex-1">
          <div ref={host} className="h-full overflow-hidden [&>div]:h-full" />
          {/* Nothing to see: a phone's keyboard only opens for a field, and this one carries what it types. */}
          <textarea
            ref={keyboard}
            onInput={onTyped}
            defaultValue={KEYBOARD_FILLER}
