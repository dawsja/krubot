...
import { boxConfig, boxUpdateStatus, forgetBoxAccounts, health, resetBox, startBoxUpdate, type BoxConfig } from "./box.ts";
...
import { activeTurns, type StopCause } from "./responder.ts";
...
async function pause(): Promise<string[]> {
  ...
  for (const turn of turns.values()) {
    ...
    turn.controller.abort("updating" satisfies StopCause);
  }
...
/**
 * Reset, like Grok Bot's "wipe and rebuild": the computer goes back to how
 * it was built. It pauses, every person's account and home on it goes, the
 * admin's home is emptied, and when the API can reach Docker the container
 * is rebuilt from its image so patches and stray installs go too. Only the
 * CLI sign-ins are kept, so nobody has to sign in to their plan again;
 * everyone's account is made again, empty, the next time they use the
 * computer. Then the API writes its skills library back, and the computer
 * comes back; a bot's home and SOUL.md are made again on its next turn.
 */
...
async function runReset(): Promise<void> {
  ...
  try {
    phase("pausing", "Pausing the computer…");
    const stopped = await pause();
    phase("resetting", "Wiping the computer (only the sign-ins for Claude Code, Codex and Grok stay)…");
    const result = await resetBox(box);
    // Nobody has an account on the box now, and the next one a person gets
    // may sit in another home.
    forgetBoxAccounts();
    log(result.removed.length ? `Removed from the admin's home: ${result.removed.join(", ")}.` : "The admin's home was already empty.");
    log(result.accounts.length ? `Accounts removed: ${result.accounts.join(", ")}. Each is made again, empty, the next time that person uses the computer.` : "No other accounts to remove.");
    const docker = dockerAvailability();
    if (docker.available) {
    ...
    await waitForBox(box, 60_000);
    forgetClaudeCheck();
    // The skills library is the API's, so it is written back at once; a
    // bot's home and its SOUL.md are made again on its next turn, and a
    // person's account on the box the next time they use the computer.
    log("Writing the skills library back…");
    await syncSkills();
    set({ phase: "done", finishedAt: new Date().toISOString() });
    log("Reset. The computer is as it was built; the bots are back with empty homes.");
    for (const threadId of stopped) postMessage({ threadId, author: "system", kind: "event", body: "The computer is back.", answered: true });
  } catch (error) {
