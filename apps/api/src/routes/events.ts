  ...
  app.get("/events", (c) => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        // A named event, not a comment: the page counts it as a sign of life,
        // so a connection that died silently (a phone asleep) is noticed.
        const keepalive = setInterval(() => send("event: ping\ndata: {}\n\n"), KEEPALIVE_MS);
        cleanup = () => {
