...
export function mcpProxyRoutes() {
  ...
  app.all("/mcp/:id/*", async (c) => {
    ...
    // Only ever under the server's own URL: the secrets and the bearer below go wherever this points.
    const search = new URL(c.req.url).search;
    const target = suffix ? proxiedUrl(server.url, suffix, search) : new URL(server.url);
    if (!target) return c.json({ error: "That path isn't under the MCP server's address" }, 400);
    if (!suffix) target.search = search;
