...
import { activeTurns, interrupt } from "../responder.ts";
...
export function threadsRoutes() {
  ...
  app.post("/threads/:id/messages", async (c) => {
    ...
    if (type.startsWith("multipart/form-data")) {
      ...
      const json = (await c.req.json().catch(() => ({}))) as { body?: string };
    ...
    const message = postMessage({ threadId: thread.id, author: "you", body, attachments });
  ...
  app.post("/threads/:id/interrupt", async (c) => {
    const stopped = await interrupt(c.req.param("id"));
