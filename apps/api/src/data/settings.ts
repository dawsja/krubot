import { DEFAULT_ENGINES, ENGINE_ACCESS, ENGINES, MODEL_ID_PATTERN, PROVIDER_KINDS, type Connection, type Engine, type EngineSettings, type Onboarding, type Settings } from "@krubot/shared";
...
export function readEngines(json: string | null, legacyModel: string | null): Record<Engine, EngineSettings> {
  ...
  for (const engine of ENGINES) {
    ...
    out[engine] = {
      ...
      provider: (PROVIDER_KINDS as readonly string[]).includes(value.provider ?? "") ? value.provider! : (out[engine].provider ?? null),
    };
  }
  return out;
}
...
export function getSettings(): Settings {
  const row = ensureKruDatabase().query("SELECT onboarding, timezone FROM kru_settings WHERE id = 1").get() as { onboarding: string; timezone: string } | null;
...
export function updateSettings(patch: Partial<Settings>): Settings {
  ...
  ensureKruDatabase()
    .query("UPDATE kru_settings SET onboarding = ?, timezone = ?, updated_at = ? WHERE id = 1")
    .run(JSON.stringify(next.onboarding), next.timezone, now());
  settingsChanged();
  return getSettings();
}
