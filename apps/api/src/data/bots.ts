...
export function createBot(input: BotInput, userId: string): Bot {
  ...
  return transaction((db) => {
    ...
    db.query(
      `INSERT INTO kru_bots (id, name, title, description, voice, color, expression, tilt, model, effort, approval, is_chief, toolkits, notify, thread_id, position, user_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      id,
      input.name,
      input.title,
      input.description,
      // Legacy columns: voice and tilt are gone from the profile; the model
      // and effort live under Settings now, for the whole team.
      "",
      input.color,
      input.expression,
      0,
