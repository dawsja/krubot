...
export function adoptOrphans(adminId: string) {
  transaction((db) => {
    ...
    db.query("UPDATE kru_skills SET user_id = ? WHERE user_id IS NULL").run(adminId);
...
export function deleteUserData(userId: string): { bots: string[] } {
  return transaction((db) => {
    ...
    db.query("DELETE FROM kru_board_cleared WHERE user_id = ?").run(userId);
    ...
    db.query("DELETE FROM kru_usage WHERE user_id = ?").run(userId);
    db.query("DELETE FROM kru_skill_files WHERE skill_id IN (SELECT id FROM kru_skills WHERE user_id = ?)").run(userId);
    db.query("DELETE FROM kru_skills WHERE user_id = ?").run(userId);
    return { bots };
  });
}
