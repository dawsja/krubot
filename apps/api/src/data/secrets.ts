 ...
 * request. Nothing here returns a value to a route, and nothing deletes
 * one: a value goes in when a bot asks for it, a newer one replaces it,
 * and deleting the person takes the lot (data/users.ts).
 */
...
/**
 * Masks every stored secret and common token format in text a bot is about
 * to post, however long. Everything a bot says passes through this or the
 * responder's redactor: a value it managed to read never reaches a message.
 */
export function hideSecrets(text: string): string {
  return createRedactor(allSecretValues(), Infinity)(text);
}
