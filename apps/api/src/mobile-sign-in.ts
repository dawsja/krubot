import { DESKTOP_SIGN_IN_CALLBACK, MOBILE_SIGN_IN_CALLBACK } from "@krubot/shared";
import { createHash, randomBytes, timingSafeEqual } from "node:crypto";

/*
 * Signing in with the OIDC provider from the apps around the web app. The
 * provider's page runs in the computer's or the phone's own browser, which
 * has passkeys and which providers trust, but its cookies aren't the app's.
 * So the app makes a verifier, sends only its hash (the challenge) to the
 * browser, and once the browser is signed in the API trades that session for
 * a one-time code the app redeems with the verifier, the way PKCE works. A
 * code caught by another app on its way back is useless without the
...
/**
 * Which app is signing in, and where its browser hands the code back. The
 * phones ("app") have a scheme of their own; the desktop has its loopback.
 */
const CALLBACKS = { app: MOBILE_SIGN_IN_CALLBACK, desktop: DESKTOP_SIGN_IN_CALLBACK } as const;
export type SignInApp = keyof typeof CALLBACKS;

/** The app named by `value`; anything else is a phone, as it was before the desktop asked. */
export function signInApp(value: unknown): SignInApp {
  return value === "desktop" ? "desktop" : "app";
}

export function signInCallback(app: SignInApp): string {
