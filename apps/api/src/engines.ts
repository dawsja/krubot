import { DEFAULT_MODEL, ENGINE_LABELS, PROVIDER_LABELS, type Engine, type Settings } from "@krubot/shared";
...
/*
 * What a bot's turn runs on, from Settings → AI: the engine (the CLI in the
 * box), its model, and how it reaches the model. On the plan, the CLI uses
 * its own sign-in on the box. On an API key, it gets the LLM proxy's URL
 * and the provider's proxy token; the key itself stays here.
 */
...
export function engineRun(settings: Settings): EngineRun {
  ...
  const token = providerProxyToken(info.provider);
  if (!provider || !token) {
