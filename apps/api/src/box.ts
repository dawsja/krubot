...
/** The box for one person's things; without a person, only for what is nobody's (update, reset, versions, skills). */
...
// ---------- the skills library, one for everyone ----------

export function listBoxSkills(config: BoxConfig): Promise<{ skills: string[] }> {
  return call({ ...config, user: null }, "GET", "/skills", undefined, 10_000);
}

export function writeBoxSkill(config: BoxConfig, slug: string, content: string): Promise<{ ok: true }> {
  return call({ ...config, user: null }, "PUT", `/skills/${slug}`, { content });
}

export async function deleteBoxSkill(config: BoxConfig, slug: string): Promise<void> {
  await call({ ...config, user: null }, "DELETE", `/skills/${slug}`);
}
...
/** Wipes the admin's home except the bots, the team's files and the CLI sign-ins; nobody else's. The box pauses itself first. */
