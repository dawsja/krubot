 ...
 * in a line, then caps its length. The cap is for log lines; a reply a
 * person reads passes `Infinity` and is kept whole.
 */
export function createRedactor(secrets: readonly (string | null | undefined)[], maxLength = MAX_LOG_LINE) {
  const known = secrets
