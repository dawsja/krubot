...
/**
 * The loopback this app listens on. `/callback` is where MCP sign-ins that
 * only allow local tools come back (Robinhood's Trading MCP), `/sign-in`
 * where signing in with the OIDC provider does. Kept in step with
 * MCP_LOOPBACK_PORT and DESKTOP_SIGN_IN_CALLBACK in packages/shared.
 */
...
/** True only for a page of the Kru Bot this app is connected to. */
function fromServerPage(event) {
  return Boolean(server && event.senderFrame?.url && sameOrigin(event.senderFrame.url, server));
}

/**
 * The sign-in with the OIDC provider that is waiting for the browser, if any:
 * `{ verifier, next }`. Only its hash goes out, so a code that comes back to
 * the loopback is worth nothing without the verifier, which stays here.
 */
let pendingSignIn = null;

/** Where to go once signed in: a path on the server, never another site. */
...
/**
 * Signing in with the OIDC provider. The window can't go to the provider
 * itself: it is another origin, so `guard` sends it to the default browser,
 * which then signs itself in and leaves the app waiting. Instead the browser
 * is asked on purpose (it has passkeys, and providers trust it) and the API
 * hands the session back here as a one-time code, the way the phone apps do.
 */
function startSignIn(next) {
  if (!server) return;
  const verifier = randomBytes(32).toString("base64url");
  pendingSignIn = { verifier, next: safePath(next) };
  const start = new URL("/api/mobile-sign-in/start", server);
  start.searchParams.set("challenge", createHash("sha256").update(verifier).digest("base64url"));
  start.searchParams.set("app", "desktop");
  openOutside(start.href);
}
...
ipcMain.handle("kru:sign-in", (event, next) => {
  if (fromServerPage(event)) startSignIn(next);
});
...
/**
 * The localhost end of a sign-in: the browser lands here and the window,
 * which is the one that holds your Kru Bot session, finishes it on the
 * server and shows where it started from.
 *
 *   /callback  an MCP server's OAuth; the code is the server's to spend
 *   /sign-in   the OIDC provider; the code becomes this window's session
 *
 * Either code is worth nothing on its own: the first needs the verifier the
 * server kept, the second the one in `pendingSignIn`. A stray request to
 * either does nothing.
 */
function startLoopback() {
  ...
  /** The page on the server that finishes what the browser brought back, or null. */
  const finishFor = (url) => {
    if (url.pathname === "/callback" && url.searchParams.get("state")) {
      const finish = new URL("/api/mcp-servers/oauth/callback", server);
      finish.search = url.search;
      return finish.href;
    }
    if (url.pathname !== "/sign-in") return null;
    const pending = pendingSignIn;
    pendingSignIn = null;
    const code = url.searchParams.get("code");
    if (!pending || !code) return new URL("/login?error=oidc", server).href;
    const redeem = new URL("/api/mobile-sign-in/redeem", server);
    redeem.searchParams.set("code", code);
    redeem.searchParams.set("verifier", pending.verifier);
    redeem.searchParams.set("next", pending.next);
    return redeem.href;
  };
  const handler = (request, response) => {
    const url = new URL(request.url ?? "/", `http://localhost:${LOOPBACK_PORT}`);
    if (request.method !== "GET" || (url.pathname !== "/callback" && url.pathname !== "/sign-in")) {
    ...
    const finish = finishFor(url);
    if (!finish) {
      response.writeHead(404, { "Content-Type": "text/plain" }).end("Not found");
      return;
    }
    void win.loadURL(finish);
